...
def create_fixtures():
    # Import form data
    form_df = pd.read_csv(f"{team_data_file_path}/PL Form Historical", index_col=False)
    # Import historical results data
    results = pd.read_csv(f"{team_data_file_path}/PL Results Historical", index_col=False)
...
def add_opponents_data():
    ...
    for season in seasons:
        team_defending = pd.read_csv(rf"{team_data_file_path}/team_{season}_defending")
        team_attacking = pd.read_csv(rf"{team_data_file_path}/team_{season}_goal-threat")
